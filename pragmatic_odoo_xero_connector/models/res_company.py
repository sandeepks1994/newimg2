...
class ResCompany(models.Model):
            ...
            if isinstance(order_lines, (dict,)):
                ...
                for i in cust.get('LineItems'):
                    if not i.get('ItemCode'):
                        ...
                        res_product = self.env['product.product'].search([('default_code', '=', i.get('ItemCode'))])
                        if res_product:
                            _logger.info('PRODUCT ======================1 : {}'.format(res_product))
                            invoice_line_vals = self.create_invoice_line(i, res_product, cust, invoice_type, tax_state)
                        else:
                            self.fetch_the_required_product(i.get('ItemCode'))
                            res_product = self.env['product.product'].search([('default_code', '=', i.get('ItemCode'))])
                            if not res_product:
                                raise ValidationError(f"Product Not Found NAME[CODE] {i.get('Description')}[{i.get('ItemCode')}]")
                            if res_product:
                                _logger.info('PRODUCT ======================2 : {}'.format(res_product))
    ...
    def create_invoice_line(self, i, res_product, cust, invoice_type, tax_state):
        _logger.info("CREATE INVOICE LINE============================",i)
        _logger.info('CREATE INVOICE LINE============================ : {}'.format(i))
        _logger.info('Product is============================ : {}'.format(res_product))
        _logger.info('tax_state is============================ : {}'.format(tax_state))
        _logger.info('invoice_type is============================ : {}'.format(invoice_type))
        ...
        _logger.info("Return INVOICE LINE============================",i)
        return dict_ol
